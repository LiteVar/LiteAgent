    ...
    <div className="pl-4">
      {childrens.map((task, index) => (
        <div key={`${task.name}-${index}`}>
          <div
            className="mt-2"
            key={`planning-task-${index}`}
          >{`(${!!letterIndex && !!letter[index] ? letter[index] : index + 1}) ${task.name}, ${task.description?.duty}, ${task.description?.constraint}`}</div>
  ...
  return (
    <div>
      ...
      <div className="px-4 py-3 rounded-lg text-sm bg-[#F5F5F5]">
        {message?.content?.taskList?.map((task: PlanningTask, index: number) => (
          <div key={`${task.name}-${index}`}>
            <div
              className={index != 0 ? 'mt-2' : ''}
              key={`planning-task-${index}`}
            >{`${index + 1}、${task.name}, ${task.description?.duty}, ${task.description?.constraint}`}</div>
        ...
        {!!message?.visible && (
          <div className="w-full flex justify-end">
            <Button
              ...
              className="px-4 py-3 mt-2 rounded-lg text-white text-base h-auto"
            >
              执行方案
            </Button>
          </div>
