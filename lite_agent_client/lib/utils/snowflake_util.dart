...
class Snowflake {
  ...
  /// 时间截向左移22位(5+5+12)
  static const int _timestampeftShift = _sequenceBits + _workerIdBits + _datacenterIdBits;
  ...
  Snowflake(this._workerId, this._datacenterId) {
    if (_workerId > _maxWorkerId || _workerId < 0) {
      throw Exception("worker Id can't be greater than $_maxWorkerId or less than 0");
    }
    if (_datacenterId > _maxDatacenterId || _datacenterId < 0) {
      throw Exception("datacenter Id can't be greater than $_maxDatacenterId or less than 0");
    }
  }
